# commands/free/role_mentionable_command.py
"""
Role mentionable command for Discord servers.

This module provides functionality to toggle whether a role is mentionable by anyone.
"""

import discord
import logging

log = logging.getLogger('MyBot.Commands.RoleMentionable')

async def execute(interaction, bot, args):
    """
    Toggles whether a Discord role is mentionable by anyone.

    Args:
        interaction (discord.Interaction): The interaction that triggered the command
        bot (commands.Bot): The bot instance
        args (dict): Command arguments
            - role (str): Role name or ID to modify
            - mentionable (str): Whether the role should be mentionable (true/false)
            - reason (str, optional): Reason for the audit log
    """
    if not interaction.guild:
        log.warning("Cannot change role mentionable status: Not in a guild context")
        await interaction.response.send_message(":warning: This command can only be used in a server.", ephemeral=True)
        return False

    # Check permissions
    if not interaction.guild.me.guild_permissions.manage_roles:
        log.warning(f"Bot lacks permission to manage roles in {interaction.guild.name}")
        await interaction.response.send_message(":warning: I don't have permission to manage roles in this server.", ephemeral=True)
        return False

    # Get parameters
    role_name_or_id = args.get('role', '')
    mentionable_str = args.get('mentionable', '')
    reason = args.get('reason', 'Role mentionable change command')

    if not role_name_or_id:
        await interaction.response.send_message(":warning: Role name or ID is required.", ephemeral=True)
        return False

    if not mentionable_str:
        await interaction.response.send_message(":warning: Mentionable value is required (true/false).", ephemeral=True)
        return False

    # Find the target role
    target_role = None
    try:
        role_id = int(role_name_or_id)
        target_role = interaction.guild.get_role(role_id)
    except (ValueError, TypeError):
        # Not an ID, try to find by name
        target_role = discord.utils.get(interaction.guild.roles, name=role_name_or_id)

    if not target_role:
        await interaction.response.send_message(f":warning: Role '{role_name_or_id}' not found.", ephemeral=True)
        return False

    # Check if the bot can manage this role (role hierarchy)
    if target_role >= interaction.guild.me.top_role:
        await interaction.response.send_message(":warning: I cannot modify a role that is higher than or equal to my highest role.", ephemeral=True)
        return False

    # Parse mentionable value
    mentionable_str = mentionable_str.lower()
    if mentionable_str in ('true', 'yes', '1'):
        mentionable = True
    elif mentionable_str in ('false', 'no', '0'):
        mentionable = False
    else:
        await interaction.response.send_message(f":warning: Invalid mentionable value: {mentionable_str}. Must be true/false.", ephemeral=True)
        return False

    await interaction.response.defer(ephemeral=True)

    try:
        # Check if the role's mentionable status is already set to the requested value
        if target_role.mentionable == mentionable:
            status = "mentionable" if mentionable else "not mentionable"
            await interaction.followup.send(f":information_source: Role `{target_role.name}` is already {status}.", ephemeral=True)
            return True

        # Change the role's mentionable status
        await target_role.edit(mentionable=mentionable, reason=reason)
        
        status = "now mentionable" if mentionable else "no longer mentionable"
        await interaction.followup.send(f":white_check_mark: Role `{target_role.name}` is {status}.", ephemeral=True)
        return True
    except discord.Forbidden:
        await interaction.followup.send(":x: I don't have permission to modify this role.", ephemeral=True)
        return False
    except discord.HTTPException as e:
        await interaction.followup.send(f":x: Failed to change role mentionable status: {e}", ephemeral=True)
        return False
